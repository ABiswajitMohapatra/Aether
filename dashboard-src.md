  ...
  <circle cx="14" cy="43" r="2.4" fill="#8a5cf6" />
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm text-[#8b93a7]">
          <span className="rounded-full border border-white/10 px-3 py-1">
            Planner â†’ Train â†’ Evaluate â†’ Analyze â†’ Decide
          </span>
          <span className="rounded-full border border-white/10 px-3 py-1">
            LLM {llm?.generative ? "live" : "policy + optional Ollama"} Â· {llm?.model ?? "adaptive"}
          </span>
        </div>
      </header>
        ...
        <main className="space-y-4">
          <section className="panel rounded-[28px] p-5">
            <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
              ...
              >
                {busy ? "Agent runningâ€¦" : "Start experiment"}
              </button>
            </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="mt-1 text-[11px] text-[#8b93a7]">
                        {item.rowCount} rows Â· {item.featureColumns.length} features Â· {item.source}
                      </p>
                    </button>
                  );
                })}
